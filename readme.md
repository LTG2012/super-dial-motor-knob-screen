...
验证码获取需要私聊作者QQ：674148718（直接发消息即可无需加好友）发送AP-wifi名称后六位，并发旋钮实物照片，证明是个人自制。作者会在一天之内回复六位验证码(拒绝提供验证码给闲鱼成品)
...
常见问题：
1. 验证码填入后切换到STA模式还是现实账号、密码、验证码页面，说明验证码填写错误
2. 切换到STA模式连接wifi直接重启，是无线路由器设置的wifi加密方式太高了，需要改成最基础的加密方式例如：WPA2
## 源码
